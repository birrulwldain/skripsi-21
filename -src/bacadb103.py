...
class c:
    ...
    def pltspec(
        ...
        for i in range(num_segments):
            ...
            # Labelkan semua puncak yang tidak dipilih
            all_peak_wavelengths = np.concatenate([peak_wavelengths, np.array([p[0] for p in selected_peaks])])
            all_peak_intensities = np.concatenate([peak_intensities, np.array([p[1] for p in selected_peaks])])
            all_peak_wavelengths, all_peak_intensities = np.unique(all_peak_wavelengths, return_index=True)
            ...
            # Tambahkan label untuk puncak yang tidak dipilih
            for i, peak_wl in enumerate(peak_wavelengths):
                if not any(np.isclose(peak_wl, p[0]) for p in selected_peaks):
                    ion_stage = "I" if nist_num[np.where(nist_wavelengths == peak_wl)[0][0]] == 1 else "II"
                    ax.scatter(peak_wl, peak_intensities[np.where(peak_wavelengths == peak_wl)[0][0]], color="green", s=8, marker=".")
                    label = f"{element} {ion_stage} {peak_wl:.6f} nm (non-selected)"
                    ax.text(
                        peak_wl,
                        peak_intensities[np.where(peak_wavelengths == peak_wl)[0][0]],
                        label,
                        fontsize=6,
                        ha="center",
                        va="bottom",
...
def main():
    ...
    while True:
        ...
        try:
            ...
            continue
        wavelengths, intensities = baca.spec(
            db_path, sample_name, iteration, lower_bound, upper_bound
        )
        if len(wavelengths) > 0 and len(intensities) > 0:
            try:
                ...
                pdf_filename = (
                    f"{sample_name}{iteration}({upper_bound}-{lower_bound})({height})({wktu}).pdf"
                )
                otpe = f"{sample_name}{iteration}({upper_bound}-{lower_bound})({height})({wktu}).txt"
                c.pltspec(
                    sample_name,
                    wavelengths,
                    intensities,
